...
# #                 "ref_1": ("IMAGE", {"forceInput": False, "tooltip": "Reference image for call 1"}),
# #                 # Slot 2 image inputs
# #                 "source_image_2": ("IMAGE", {"forceInput": False, "tooltip": "Source image for call 2"}),
# #                 "ref_2": ("IMAGE", {"forceInput": False, "tooltip": "Reference image for call 2"}),
# #                 # Slot 3 image inputs
# #                 "source_image_3": ("IMAGE", {"forceInput": False, "tooltip": "Source image for call 3"}),
# #                 "ref_3": ("IMAGE", {"forceInput": False, "tooltip": "Reference image for call 3"}),
# #                 # Slot 4 image inputs
# #                 "source_image_4": ("IMAGE", {"forceInput": False, "tooltip": "Source image for call 4"}),
# #                 "ref_4": ("IMAGE", {"forceInput": False, "tooltip": "Reference image for call 4"}),
# #                 # Slot 5 image inputs
# #                 "source_image_5": ("IMAGE", {"forceInput": False, "tooltip": "Source image for call 5"}),
# #                 "ref_5": ("IMAGE", {"forceInput": False, "tooltip": "Reference image for call 5"}),
# #             },
# #         }
...
# #     def GENERATE_MODE_FUNCTION(self, api_key: str, prompt: str, slot_id: int, temperature: float = 0.7, enable_safety: bool = True) -> Tuple[torch.Tensor, str]:
# #         """Generate mode - Text-to-Image: Generate high-quality images from text descriptions only"""
# #         try:
...
# #                     return _placeholder(), f"Slot {slot_id}: {part.text}"
# #                 elif part.inline_data is not None:
# #                     try:
# #                         pil = _decode_image_bytes(part.inline_data.data)
# #                         tensor = self._stack([pil])
# #                         return tensor, f"Slot {slot_id}: Successfully generated 1 image"
# #                     except Exception as e:
# #                         return _placeholder(), f"Slot {slot_id}: Decode error: {e}"

# #             return _placeholder(), f"Slot {slot_id}: No images were generated"

# #         except ImportError:
# #             return _placeholder(), f"Slot {slot_id}: google.genai not installed. Install with: pip install google-genai pillow numpy"
# #         except errors.APIError as e:
# #             return _placeholder(), f"Slot {slot_id}: APIError {getattr(e,'code','')}: {getattr(e,'message',str(e))}"
# #         except Exception as e:
# #             return _placeholder(), f"Slot {slot_id}: Error: {e}"

# #     def IMAGE_EDIT_FUNCTION(self, api_key: str, prompt: str, source_img: torch.Tensor, slot_id: int, temperature: float = 0.7, enable_safety: bool = True) -> Tuple[torch.Tensor, str]:
# #         """Image edit mode - Edit existing images with text prompts"""
# #         try:
# #             from google import genai
...
# #             image = _tensor_to_pil(source_img)
...
# #                     tensor = self._stack([image])
# #                     return tensor, f"Slot {slot_id}: Successfully edited 1 image"
...
# #     def STYLE_TRANSFER_FUNCTION(self, api_key: str, prompt: str, source_img: torch.Tensor, ref_img: torch.Tensor, slot_id: int, temperature: float = 0.7, enable_safety: bool = True) -> Tuple[torch.Tensor, str]:
# #         """Style transfer mode - Transfer style from reference image to source image"""
# #         try:
# #             from google import genai
# #             from google.genai import types
# #             from PIL import Image
...
# #             source_image = _tensor_to_pil(source_img)
# #             reference_image = _tensor_to_pil(ref_img)

# #             # Create semantic prompt that clarifies image roles
# #             semantic_prompt = f"Image 1 is the source image to edit. Image 2 is the reference image to derive the edit from. {prompt}"

# #             response = client.models.generate_content(
# #                 model="gemini-2.5-flash-image-preview",
# #                 contents=[semantic_prompt, source_image, reference_image],
# #                 config=types.GenerateContentConfig(temperature=temperature),
# #             )
...
# #                     image = Image.open(BytesIO(part.inline_data.data))
# #                     tensor = self._stack([image])
# #                     return tensor, f"Slot {slot_id}: Successfully transferred style"
...
# #     def OBJECT_INSERTION_FUNCTION(self, api_key: str, prompt: str, source_img: torch.Tensor, ref_img: torch.Tensor, slot_id: int, temperature: float = 0.7, enable_safety: bool = True) -> Tuple[torch.Tensor, str]:
# #         """Object insertion mode - Insert objects from reference image into source image"""
# #         try:
# #             from google import genai
# #             from google.genai import types
# #             from PIL import Image
...
# #             # Create semantic prompt that clarifies image roles
# #             semantic_prompt = f"Image 1 is the source image to edit. Image 2 is the reference image containing objects to insert. {prompt}"
...
# #                     image = Image.open(BytesIO(part.inline_data.data))
# #                     tensor = self._stack([image])
# #                     return tensor, f"Slot {slot_id}: Successfully inserted objects"
...
# #             operation: str = "generate", temperature: float = 0.7, enable_safety: bool = True,
# #             source_image_1=None, ref_1=None,
# #             source_image_2=None, ref_2=None,
# #             source_image_3=None, ref_3=None,
# #             source_image_4=None, ref_4=None,
# #             source_image_5=None, ref_5=None,
...
# #         if parallels_share_inputs:
# #             # Shared inputs - use individual prompts but shared images
# #             prompts = [prompt_1, prompt_2, prompt_3, prompt_4, prompt_5]
            
# #             for i in range(in_parallel):
# #                 slot_data.append({
# #                     'prompt': prompts[i] or prompt_1,
...
# #             refs = [ref_1, ref_2, ref_3, ref_4, ref_5]
            
# #             for i in range(in_parallel):
# #                 slot_id = i + 1
# #                 individual_prompt = prompts[i] or prompt_1
...
# #                             self.GENERATE_MODE_FUNCTION,
# #                             key, slot['prompt'], slot['slot_id'], temperature, enable_safety
# #                         )
# #                     elif operation == "edit":
# #                         future = executor.submit(
# #                             self.IMAGE_EDIT_FUNCTION,
# #                             key, slot['prompt'], slot['source'], slot['slot_id'], temperature, enable_safety
# #                         )
# #                     elif operation == "style_transfer":
# #                         future = executor.submit(
# #                             self.STYLE_TRANSFER_FUNCTION,
# #                             key, slot['prompt'], slot['source'], slot['ref'], slot['slot_id'], temperature, enable_safety
# #                         )
# #                     else:  # object_insertion
# #                         future = executor.submit(
# #                             self.OBJECT_INSERTION_FUNCTION,
# #                             key, slot['prompt'], slot['source'], slot['ref'], slot['slot_id'], temperature, enable_safety
# #                         )
...
# # nano_banana_parallel.py
# # Nano Banana (Gemini 2.5) — Parallel (max 5)
# # - Parallel fan-out 1..5 via in_parallel
# # - parallels_share_inputs toggle
# # - Images optional; generation works prompt-only
# # - Edit/insert/style without source: skip that slot (placeholder + log)
# # - Outputs: result_1..result_5 + operation_log
...
#             raw2 = base64.b64decode(raw, validate=False)
#             if _looks(raw2):
#                 return Image.open(BytesIO(raw2)).convert("RGB")
#         except:
#             pass
...
# def _pil_part(pil: Image.Image, max_side=1024, mime="image/jpeg"):
#     # lazy import keeps file usable even if google-genai isn't installed
#     from google.genai import types
#     pil = pil.convert("RGB")
#     w, h = pil.size
#     s = max(w, h)
...
# def _prep_refs(*imgs) -> List[Image.Image]:
#     out = []
#     for im in imgs:
...
# def _stack(pils: List[Image.Image]) -> torch.Tensor:
#     if not pils:
#         return _placeholder()
...
# class NanoBananaParallel:
#     """
#     Max parallel slots = 5.
#     - parallels_share_inputs = True  -> use only *_1 inputs for all active slots
#     - parallels_share_inputs = False -> use per-slot *_i inputs up to in_parallel
#     All images are OPTIONAL. Generation can be prompt-only.
#     """
...
#     FUNCTION = "run"
#     CATEGORY = "NanoBanana (Parallel)"
#     DESCRIPTION = "Gemini 2.5 Flash Image (preview) with parallel execution (1–5). Images optional."

#     @classmethod
#     def INPUT_TYPES(cls):
#         """
#         ComfyUI shows all declared inputs; we can't hide groups conditionally in core.
#         Extra slots are optional and simply ignored when not used.
#         """
#         required = {
#             "api_key": ("STRING", {"default": "", "password": True}),
#             "in_parallel": ("INT", {"default": 1, "min": 1, "max": 5, "step": 1}),
#             "parallels_share_inputs": ("BOOLEAN", {"default": True}),
#             "prompt_1": ("STRING", {"default": "Generate a high-quality, photorealistic image", "multiline": True}),
#         }

#         # all images optional; refs always optional
#         optional = {
#             # slot 1 (shared in share mode)
#             "source_image_1": ("IMAGE", {"forceInput": False}),
#             "ref_a_1": ("IMAGE", {"forceInput": False}),
#             "ref_b_1": ("IMAGE", {"forceInput": False}),
...
#         # per-slot prompts+images (2..5) — optional; ignored if sharing or if in_parallel < slot index
#         for i in range(2, 6):
#             optional[f"prompt_{i}"] = ("STRING", {"default": "Generate a high-quality, photorealistic image", "multiline": True})
#             optional[f"source_image_{i}"] = ("IMAGE", {"forceInput": False})
#             optional[f"ref_a_{i}"] = ("IMAGE", {"forceInput": False})
#             optional[f"ref_b_{i}"] = ("IMAGE", {"forceInput": False})

#         return {"required": required, "optional": optional}

#     @classmethod
#     def VALIDATE_INPUTS(cls, **kwargs):
#         # Nothing is mandatory at the node level; per-slot checks are done at runtime.
#         return True
...
#         api_key: str,
#         prompt: str,
#         source_img: Optional[torch.Tensor],
#         ref_a: Optional[torch.Tensor],
#         ref_b: Optional[torch.Tensor],
#         operation: str,
#         temperature: float,
#         batch_count: int,
#         enable_safety: bool,
...
#             # Edit-like ops without a source: skip this slot only.
#             if operation in ["edit", "style_transfer", "object_insertion"] and source_img is None:
#                 return _placeholder(), f"Slot {slot_id}: '{operation}' needs a source image; skipping this slot."

#             parts = [types.Part.from_text(text=prompt)]

#             if source_img is not None:
#                 parts.append(_pil_part(_tensor_to_pil(source_img), 1024, "image/jpeg"))

#             for ref_pil in _prep_refs(ref_a, ref_b):
...
#                                         found += 1
#                                     except Exception as e:
#                                         log.append(f"Slot {slot_id} batch {j+1} decode fail: {e}")
#                     log.append(f"Slot {slot_id} batch {j+1}: Generated {found} images")
#                 except errors.APIError as e:
#                     log.append(f"Slot {slot_id} batch {j+1} APIError {getattr(e,'code','')}: {getattr(e,'message',str(e))}")
#                 except Exception as e:
#                     log.append(f"Slot {slot_id} batch {j+1} error: {e}")
...
#         except ImportError:
#             return _placeholder(), f"Slot {slot_id}: google.genai not installed. Install: pip install google-genai pillow numpy"
#         except Exception as e:
...
#         prompt_1: str = "Generate a high-quality, photorealistic image",
...
#         K = max(1, min(int(in_parallel), 5))

#         # build slot inputs
#         slot_data = []
#         if parallels_share_inputs:
#             source = kwargs.get("source_image_1")
#             ref_a = kwargs.get("ref_a_1")
#             ref_b = kwargs.get("ref_b_1")
#             prompt_base = prompt_1
...
#                 slot_data.append({
#                     "slot_id": i,
#                     "prompt": kwargs.get(f"prompt_{i}", prompt_1),
#                     "source": kwargs.get(f"source_image_{i}"),
#                     "ref_a": kwargs.get(f"ref_a_{i}"),
#                     "ref_b": kwargs.get(f"ref_b_{i}"),
#                 })
...
#                         s["slot_id"],
#                     )
#                     future_to_slot[fut] = s["slot_id"]

#                 # collect in slot order
#                 slot_results: Dict[int, Tuple[torch.Tensor, str]] = {}
#                 for fut in as_completed(future_to_slot):
#                     sid = future_to_slot[fut]
#                     try:
#                         tensor, log = fut.result()
#                         slot_results[sid] = (tensor, log)
#                     except Exception as e:
#                         slot_results[sid] = (_placeholder(), f"Slot {sid}: Execution error: {e}")

#                 for i in range(1, K + 1):
#                     if i in slot_results:
#                         tensor, log = slot_results[i]
...
#         combined_log = f"Parallel execution completed ({K} slots):\n" + "\n".join(logs)
#         return tuple(results + [combined_log])


# # Node registration
# NODE_CLASS_MAPPINGS = {"NanoBananaParallel": NanoBananaParallel}
# NODE_DISPLAY_NAME_MAPPINGS = {"NanoBananaParallel": "Nano Banana (Gemini 2.5) — Parallel (max 5)"}
# nano_banana_parallel.py
# Parallel ComfyUI node for Gemini 2.5 Flash Image (preview)
# - Supports 1-9 parallel calls with shared or individual inputs
import base64
