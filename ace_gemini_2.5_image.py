# -*- coding: utf-8 -*-
# DROP-IN replacement for: ACE_gemini_flash_image_2_5
# Robust inline_data handling (auto base64 decode even when bytes), safe streaming, non-stream fallback.
...
def _looks_like_b64_bytes(b: bytes) -> bool:
    # Heuristic: mostly base64 charset, length reasonable, not starting with known image magics
...
def _normalize_inline_data(data) -> Optional[bytes]:
    # Accept str (b64), bytes (raw or b64), bytearray, memoryview
    if data is None:
        return None
...
class ACE_gemini_flash_image_2_5:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "prompt": ("STRING", {"multiline": True, "default": "Describe or edit the image..."}),
            },
            "optional": {
                "images": ("IMAGE",),
    ...
    def _build_contents(self, prompt: str, images: Optional[torch.Tensor]):
        parts = []
        if images is not None:
            try:
                parts.append(
                    gtypes.Part.from_bytes(
                        data=_tensor_to_png_bytes(images),
                        mime_type="image/png",
                    )
                )
            except Exception:
    ...
    def run(self,
        ...
        # Always ask for text+image; image-only is flaky with this model.
        ...
        if not tensors:
            ...
            extras = []
            if decode_warnings:
                extras.append(" | ".join(decode_warnings))
            if saved_paths:
                extras.append("saved raw bytes: " + ", ".join(saved_paths))
            if not force_image_only and texts:
                extras.append(" ".join(texts))
...
NODE_CLASS_MAPPINGS = {"ACE_gemini_flash_image_2_5": ACE_gemini_flash_image_2_5}
NODE_DISPLAY_NAME_MAPPINGS = {"ACE_gemini_flash_image_2_5": "Gemini 2.5 Flash Image (Fixed)"}
